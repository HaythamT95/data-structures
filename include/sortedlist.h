...
/*
Function Description: Create a list, with given
                      compare function for element inserting.
...
Undefined behaviour: if sorted_list is NULL
...
Undefined behaviour: if iter holds null address
...
Return value: sorted_list_iter_ty next iterator of to_remove

Undefined behaviour: if 'list' is NULL or 'to_remove' holds null address 
                      or next iterator is null
...
Undefined behaviour: if list, cmp_func or element, output are NULL pointers.
Note:The function expects the cmp_func function to return 0 in case of success
...
Note: src is empty after using this function
